...
def create_roles():
    ...
    if not Role.query.filter_by(name='subscriber').first():
        ...
        print(__name__, '   Role created:', thisrole)
    db.session.commit()
    print(__name__,"   {0} default roles created in database {1}".format(created,DATABASE_NAME))

def create_departments():
    print(__name__, 'create Departments')
    created = 0
    if not Department.query.filter_by(name='development').first():
        ...
        created = created + 1
        print(__name__, '   Department created:', thisDpmt)
    ...
    if not Department.query.filter_by(name='').first():
        ...
        print(__name__, '   Department created:', thisDpmt)
    db.session.commit()
    print(__name__,"   {0} Departments created in database {1}".format(created,DATABASE_NAME))

def create_users():
    print(__name__, 'create default Users')
    created = 0
    users = [
    ...
    for user in users:
        ...
        if not User.query.filter_by(email=email).first():
            ...
            created = created + 1
            print(__name__, '   user created:', thisUser)
    db.session.commit()
    print(__name__,"   {0} Users created in database {1}".format(created,DATABASE_NAME))
